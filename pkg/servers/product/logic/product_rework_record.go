...
func QueryProductReworkRecord(req *proto.QueryProductReworkRecordRequest, resp *proto.QueryProductReworkRecordResponse, preload bool) {
	db := model.DB.DB().Model(&model.ProductReworkRecord{}).Preload("ProductionStation").Preload("ProductionStation.ProductionLine").Preload("ProductInfo").Preload("ProductInfo.ProductOrder")
